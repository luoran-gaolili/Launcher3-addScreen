...
//add bug launcher3 error by luoran 20170526(start)
import java.util.LinkedHashSet;
import java.util.Set;
//add bug launcher3 error by luoran 20170526(end)
...
public class LauncherProvider extends ContentProvider {
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        @Thunk int loadFavorites(SQLiteDatabase db, AutoInstallsLayout loader) {
			...
			//modify bug launcher3 error by luoran 20170526(start)
			Set<Long> screenIdsSet = new LinkedHashSet<Long>();
			screenIdsSet.addAll(screenIds);
			screenIds.clear();
			screenIds.addAll(screenIdsSet);
			//add bug launcher3 error by luoran 20170526(end)
            ...
            for (Long id : screenIds) {
				...
				//modify bug launcher3 error by luoran 20170526(start)
				if(updateWorkSpaceScreens(db, id, rank)){
                ...
                //            + "from default layout");
                //}
                //modify bug launcher3 error by luoran 20170526(end)
                rank++;
            }
		...
		//add bug launcher3 error by luoran 20170526(start)
		private boolean updateWorkSpaceScreens(SQLiteDatabase db, long id, int rank){
		    boolean needinsert = true;
			Cursor c = null;
			try{
				c = db.rawQuery("select * from " + TABLE_WORKSPACE_SCREENS + " where " + LauncherSettings.WorkspaceScreens._ID + "=" + id + " limit 0,1", null);
				if(c != null && c.getCount() >0){
				  needinsert = false;
					db.execSQL(("update " + TABLE_WORKSPACE_SCREENS + " set " + LauncherSettings.WorkspaceScreens.SCREEN_RANK + "=? where " + LauncherSettings.WorkspaceScreens._ID + "=?")
						, new String[]{rank+"", id+""});
				}
			} catch (Throwable e){e.printStackTrace();}finally{if(c!=null)c.close();}
			return needinsert;
		}
		//add bug launcher3 error by luoran 20170526(end)

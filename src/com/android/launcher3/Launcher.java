...
//A:EWWJLL-659 xiaochen.liu 20170815 (start)
import android.telephony.SubscriptionManager;
import android.telephony.SubscriptionInfo;
//A:EWWJLL-659 xiaochen.liu 20170815 (end)
...
//add by luoran for low storage set wallpaper error(start)
import android.app.ActivityThread;
import android.content.pm.IPackageManager;
import android.os.RemoteException;
//add by luoran for low storage(end)
//add by luoran for rgk MHL(start)
import android.os.RemoteException;
...
import com.android.launcher3.projector.SubActionButton;
import com.android.launcher3.projector.SubMenuView;
import android.view.animation.AccelerateDecelerateInterpolator;
import android.support.annotation.NonNull;
import com.android.launcher3.projector.ShimmerTextView;
import android.telephony.TelephonyManager;
import android.database.Cursor;
import com.android.launcher3.projector.CircleImageView;
import android.media.ThumbnailUtils;
import android.provider.MediaStore;
import android.content.ContentResolver;
//add by luoran for rgk MHL(end)
/**
...
public class Launcher extends Activity
    ...
    //add by luoran for rgk MHL(start)
    public static final String MHL_LANDSCAPE = "mhl_landscape";// Settings.System.MHL_LANDSCAPE
    //add by luoran for rgk MHL(end)
    ...
    void setOrientation() {
        if (mRotationEnabled) {
            ...
            //modify by luoran for mhl orientation(start)
            if (!getResources().getBoolean(R.bool.is_support_mhl)) {
            ...
            //modify by luoran for mhl orientation(end)
        }
    }
    ...
    //add unread feature by zhaopenglin DWEQLSY-351 start
    ...
    //add unread feature by zhaopenglin DWEQLSY-351 end
    ...
    //add by kangduohua for N default_lock_wallpaper(start)
    private static final String DEFAULT_LOCK_WALLPAPER="com.ragentek.default.lock.wallpaper";
    //add by kangduohua for N default_lock_wallpaper(end)

	//A:EWWJLL-659 xiaochen.liu 20170815 (start)
	public static String[] mSimName = new String[2];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //add by kangduohua for N default_lock_wallpaper(start)
        if(getResources().getBoolean(R.bool.support_default_lock_wallpaper)){
            if(Settings.Secure.getInt(getContentResolver(),DEFAULT_LOCK_WALLPAPER,0)==0){
                new SetLockWallpaperTask().execute();
            }
        }
        //add by kangduohua for N default_lock_wallpaper(end)
...
//            UnreadContentObserver.getUnreadContentObserver(this).getUnreadMms();
        ...
        Log.i(TAG, "getSimName");
        getSimName();
        //A:EWWJLL-659 xiaochen.liu 20170815 (end)
    }
       //add by kangduohua for N default_lock_wallpaper(start)
    class SetLockWallpaperTask extends AsyncTask<Void, Void, Void> {
        @Override
        protected Void doInBackground(Void... params) {
            try{
                Bitmap defaultLockWallpaper=BitmapFactory.decodeResource(getResources(), R.drawable.keyguard_wallpaper);
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                defaultLockWallpaper.compress(Bitmap.CompressFormat.PNG, 100, baos);
                InputStream is = new ByteArrayInputStream(baos.toByteArray());
                NycWallpaperUtils.setStream(Launcher.this,is,null,true,WallpaperManager.FLAG_LOCK);
             Log.d("LUORAN","success");
               }catch(Exception e){
                    e.printStackTrace();
            //return false;
            }
 ...
 //add by luoran for N default_lock_wallpaper(end)
    ...
    //modify by luoran for rgk mhl(start)
   /* protected void populateProjectorContentContainer() {
    ...
    //modify by luoran for rgk mhl(end)
...
//            projector_tip.setVisibility(View.INVISIBLE);
    ...
    public void onClick(View v) {
        ...
        if(getResources().getBoolean(R.bool.is_positivo_launcher)){
        ...
        if (v instanceof CellLayout) {
            ...
            //add by luoran for rgk MHL (start)
            if(getResources().getBoolean(R.bool.is_support_mhl)){
                if(mCircleMenu != null && galleryMenu != null && docMenu != null){
                if(galleryMenu.isMenuOpened() || docMenu.isMenuOpened()){
                    return;
                    }
                mCircleMenu.extendClose(true);
                }
    ...
    protected void onClickWallpaperPicker(View v) {
        ...
        // modify by luoran for low storage start
        if(!isStorageLow()){
        ...
        // modify by luoran for low storage end
        if (mLauncherCallbacks != null) {
            mLauncherCallbacks.onClickWallpaperPicker(v);
        }
    }
    // add by luoran for low storage start
    ...
    protected void onClickSettingsButton(View v) {
        if (LOGD) Log.d(TAG, "onClickSettingsButton");
         //Add BUG_ID:DWYQJ-531 luoran 20170522 start
        ...
        //Add BUG_ID:DWYQJ-531 luoran 20170522 end
        if (mLauncherCallbacks != null) {
    ...
    public boolean onLongClick(View v) {
        ...
        //add by luoran for rgk MHL(start)
        if(getResources().getBoolean(R.bool.is_support_mhl)){
            if(getDeviceProfile().isLandscape){
                return false;
            }
        }
        //add by luoran for rgk MHL(end)
    ...
    private boolean showAppsOrWidgets(State toState, boolean animated, boolean focusSearchBar) {
        ...
        mUserPresent = false;
        updateAutoAdvanceState();
        //modify by luoran for modify animation EWWJLL-1125 (start)
	...
	//add by luoran for modify animation EWWJLL-1125 (start)
    public void closeFolder(boolean closeFolder,boolean isShowWSH) {
    ...
    //add by luoran for modify animation EWWJLL-1125 (end)
    ...
    //add by luoran for positivo launcher(start)
    ...
    //add by luoran for positivo launcher(end)
    /**
    ...
    public View getOrCreateQsbBar() {
        // Add BUG_ID:DWYQH-404 luoran 20170220(start)
        if(getResources().getBoolean(R.bool.is_rm_qsb)) return null;
        // Add BUG_ID:DWYQH-404 luoran 20170220(end)
        ...
        if(mProjectorContent != null){
            ...
            mProjectorMenu.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    startActivity(new Intent().setClassName("com.android.mhlsettings", "com.android.mhlsettings.DialogActivity"));
                    return true;
                }
            });
        }
...
//        intent.putExtra("titleName", getResources().getString(R.string.menu_doc));
   ...
   private void openFileManager() {
    ...
    private void setupFloatingButton() {
        final boolean isLand = getDeviceProfile().isLandscape;
    ...
    private void observerGalleryDataDb() {
        galleryDataObserver = new ContentObserver(new Handler()) {
            ...
            public void onChange(boolean selfChange) {
                ...
                mHandler.postDelayed(mContentObserverChange, 5000);
...
//                subGallery2.setImageBitmap(getImageThumbnail(picture, subParams.width, subParams.height));
    ...
    private void setupMainMenu(boolean isLand) {
        ...
        mDoc.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
            try{
                Intent intent = new Intent();
                ...
                e.printStackTrace();
                Toast.makeText(getApplication(), R.string.no_file, Toast.LENGTH_SHORT).show();
            }
                Intent i = new Intent("com.qingcheng.home.startmini");
                sendBroadcast(i);
        ...
        mGallery.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
            try{
                 if(getResources().getBoolean(R.bool.is_support_gallary)){
                    Intent intent = new Intent();
                    ...
                    e.printStackTrace();
                    Toast.makeText(getApplication(), R.string.no_gallary, Toast.LENGTH_SHORT).show();
        ...
        mGame.setOnClickListener(new OnClickListener() {
            ...
            try{
                    ...
                    e.printStackTrace();
                    Toast.makeText(getApplication(), R.string.no_game, Toast.LENGTH_SHORT).show();
                }
            }
        });
         mSettings.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent();
                intent.setClassName("com.android.mhlsettings", "com.android.mhlsettings.AllSettingsActivity");
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(intent);
            }
        });
        ...
        mVideo.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //M by qiang.li bug_id:EWWJLL-638 20170802 start
                /*Intent intent = new Intent();
                ...
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(intent);
                //M by qiang.li bug_id:EWWJLL-638 20170802 end
                Intent i = new Intent("com.qingcheng.home.startmini");
                sendBroadcast(i);
            }
        });
    ...
    private void openMHLSettings() {
        startActivity(new Intent().setClassName(
                ...
                "com.android.mhlsettings.SettingsActivity"));
    }
     ...
     private void observerFileDataDb() {
        ...
        fileDataObserver = new ContentObserver(new Handler()) {
            ...
            public void onChange(boolean selfChange) {
                ...
                mHandler.postDelayed(mFileContentObserverChange, 5000);
    ...
    public void bindAppWidget(final LauncherAppWidgetInfo item) {
        ...
        LauncherAppWidgetProviderInfo appWidgetInfo;
        if(getResources().getBoolean(R.bool.is_support_mhl)){
            if (item.providerName != null && item.screenId == 1) {
    ...
    //                    item.cellY, item.spanX, 2, false);
    //            addWidgetToAutoAdvanceIfNeeded(item.hostView, appWidgetInfo);
    //            //add small weather widget
    //            AppWidgetProviderInfo smallWidgetInfo = mModel.findAppWidgetProviderInfoWithComponent(this, new ComponentName("com.greenorange.weather", "com.greenorange.weather.wdiget.TimeWeatherWidgetOneLine"));
    //            PendingAddWidgetInfo smallPendingInfo = new PendingAddWidgetInfo(smallWidgetInfo, null, null);
    //            smallPendingInfo.spanX = item.spanX;
    //            smallPendingInfo.spanY = 1;
    //            smallPendingInfo.minSpanX = item.minSpanX;
    //            smallPendingInfo.minSpanY = 1;
    //            Bundle smallOptions =
    //                    AppsCustomizePagedView.getDefaultOptionsForWidget(this, smallPendingInfo);
    //
    //            int smallId = mAppWidgetHost.allocateAppWidgetId();
    //            boolean success = mAppWidgetManager.bindAppWidgetIdIfAllowed(
    //                    smallId, smallWidgetInfo, smallOptions);
                ...
                return;
            }else if(item.providerName != null && item.screenId == 1){
                ...
                if (mProjectorContent != null) {
                   ...
                   // RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                    //.addView(item.hostView, layoutParams);
                } else {
                    mProjectorContent = LayoutInflater.from(this).inflate(R.layout.projector_panel, null, false);
                    CellLayout celllayout = getCellLayout(item);
                    int spanX = celllayout.getCountX();
   ...
   /**
    	...
    	mSimName = new String[2];
         ...
         if(subscriptionInfos != null){
        	 if (subscriptionInfos.size() == 0) {
        		 //mSimName = null;
        		 mSimName[0] = mSimName[1] = "SIM Toolkit";
        	 } else if (subscriptionInfos.size() == 1) {
    			mSimName[0] = subscriptionInfos.get(0).getDisplayName() + "";
    			mSimName[1] = subscriptionInfos.get(0).getDisplayName() + "";
    		 } else if (subscriptionInfos.size() == 2) {
    			mSimName[0] = subscriptionInfos.get(0).getDisplayName() + "";
    			mSimName[1] = subscriptionInfos.get(1).getDisplayName() + "";
    		}
        	if (mSimName != null) {
                Log.i("RGKLauncher F","mSimName[0] = "+mSimName[0]+",mSimName[1] = "+mSimName[1]);
        		mSimName[0] = getWangtedName(mSimName[0]);
        		mSimName[1] = getWangtedName(mSimName[1]);
                Log.i("RGKLauncher S","mSimName[0] = "+mSimName[0]+",mSimName[1] = "+mSimName[1]);
        	}
         }else{
            mSimName[0] = mSimName[1] = "SIM Toolkit";
         }
    ...
    public String getWangtedName(String name){
        if((name.length() - 3) < 1) return name; 
    	if (name.substring(0, name.length() - 3).equalsIgnoreCase("Claro")) return "Menu Claro";
    	if (name.substring(0, name.length() - 3).equalsIgnoreCase("Oi")) return "Mundo Oi";
    	if (name.substring(0, name.length() - 3).equalsIgnoreCase("TIM BRASIL")) return "Serviços TIM";
    	if (name.substring(0, name.length() - 3).equalsIgnoreCase("VIVO")) return "Vivo Chip";
    	return name;
    }
	//A:EWWJLL-659 xiaochen.liu 20170815 (end)
}
